...
$$\psi(P_{0})\,=\,\sum_{w}\,\left[\sum_{y}\,y\,P(Y=y\mid A=1,W=w)-\,\sum_{y}\,y\,P(Y = y\mid A=0,W=w)\right]P(W=w)$$
...
The fluctuation parameter $\hat{\epsilon}$ is estimated using maximum likelihood
by setting $\text{logit}(\bar{Q^{0}}(A,W))$ as an offset in a intercept-free
logistic regression with the **clever covariate** $H(A,W)$ as independent variable:

$$\bar{Q^{1}}(A,W)\,=\,\text{expit}\left[\text{logit}\left(\bar{Q^{0}}(A, W)\right)\,+\,\hat{\epsilon}\,H(A,W)\right] \qquad (5)$$

$$\bar{Q^{1}}(0,W)\,=\,\text{expit}\left[\text{logit}\left(\bar{Q^{0}}(0,W)\right)\,+\,\hat{\epsilon}\,H(0,W)\right]$$

$$\bar{Q^{1}}(1,W)\,=\,\text{expit}\left[\text{logit}\left(\bar{Q^{0}}(1,W)\right)\,+\,\hat{\epsilon}\,H(1,W)\right]$$
Where,
$$H(A,W)\,=\,\frac{I(A=1)}{\hat{g}(1|W)}-\frac{I(A=0)}{\hat{g}(0|W)}$$
is the stabilized inverse probability of treatment (A) weight (IPTW),
namely the **clever covariate** and **I** defines an indicator
function (note that $\hat{g}(A|W)$ is estimated from step 2).
...
updated by the substitution parameter $\hat{\epsilon}$. The substitution update
...
For $\Psi(\bar{Q_{n}}^{*})$, the updated estimate of the potential
outcomes only needs one iteration $\Psi(\bar{Q_{n}}^{*})$ from
$\bar{Q}^{0}(A,W)\,=>\bar{Q^{1}}(A,W)$. Therefore, model (5) targets
$E[\hat{Y}_{A=0}]\;\text{and}\; E[\hat{Y}_{A=1}]$ simultaneously via
the clever covariate $H(A,W)$. Hence
